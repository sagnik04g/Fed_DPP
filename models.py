...
class Resnet18_mnist(torch.nn.Module):
    ...
    def __init__(self, args: object, num_class: int = 10, freeze: bool = True) -> None:
        ...
        if len(resnet18_list) == 0:
            resnet18 = torch.hub.load('pytorch/vision:v0.10.0', 'resnet18', pretrained=False)
            resnet18.fc = torch.nn.Identity() # remove last FC layer
            for p in resnet18.parameters(): # freeze resnet50
                p.requires_grad = False
            resnet18.to(device)
            resnet18_list.append(resnet18)
        assert(len(resnet18_list) == 1)
        
        self.logits = torch.nn.Linear(in_features = 512, out_features = num_class)
        if(args.project=='mnist'):
